...
### 9.4 Planning 模式不兼容 Flow-only

`Workflow.planning.enabled` 需要 Hero 来执行 Planner Agent。Flow-only 模式（`hero = None`）下 Planning 不可用。若 `planning.enabled = true` 且 `hero.is_none()`，`validate()` 应报错。混合模式（`hero` 和 `flow_executor` 都非 None）下 Planning 可用。

### 9.5 共享取消模式变更

当前 `FlowHandleRef` 支持跨 task Clone 并调用 `cancel()`。V0.4 用 `ExecutionHandle` 替代后，取消需要持有 handle 本身（不可 Clone 共享）。如果需要共享取消能力，调用方需自行包装 `Arc<AtomicBool>` + 轮询。
